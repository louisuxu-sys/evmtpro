...
// ===== 追蹤確認卡片（跟隨後顯示）=====
function buildTrackingFlex(engine, mtInfo) {
  const state = engine.getState();
  const tableName = state.tableName || engine.tableName || '百家樂';
  const dealer    = state.dealer || '-';

  // 統計
  const s = mtInfo && mtInfo.summary;
  const B = s ? (s.Banker || 0) : (state.bankerWins || 0);
  const P = s ? (s.Player || 0) : (state.playerWins || 0);
  const T = s ? (s.Tie   || 0) : (state.tieWins    || 0);
  const total = B + P + T || 1;

  // 預測
  const pred  = state.prediction;
  const predW = pred ? (pred.winner === 'B' ? '莊' : pred.winner === 'P' ? '閒' : '和') : null;
  const predC = pred ? Math.round((pred.confidence || 0) * 100) : null;
  const predColor = pred ? (pred.winner === 'B' ? COLOR_B : pred.winner === 'P' ? COLOR_P : COLOR_T) : '#888';

  function statCol(label, count, color) {
    return {
      type: 'box', layout: 'vertical', alignItems: 'center', flex: 1,
      contents: [
        { type: 'text', text: label, color, size: 'sm', weight: 'bold', align: 'center' },
        { type: 'text', text: String(count), color, size: 'xl', weight: 'bold', align: 'center' },
        { type: 'text', text: `${Math.round(count / total * 100)}%`, color: '#888', size: 'xxs', align: 'center' }
      ]
    };
  }
  ...
  if (predW) {
    bodyContents.push({
      ...
      contents: [
        { type: 'text', text: '下手預測', size: 'sm', color: '#888', flex: 2 },
        { type: 'text', text: predW, size: 'xl', weight: 'bold', color: predColor, flex: 1, align: 'center' },
        { type: 'text', text: `${predC}%`, size: 'sm', color: '#888', flex: 1, align: 'end' }
      ]
    });
  } else {
    bodyContents.push({
      type: 'text', text: '等待開局資料…', size: 'sm', color: '#888', margin: 'md', align: 'center'
    });
  }
  ...
  bodyContents.push({
    ...
    contents: [{
      ...
      color: COLOR_HEADER,
      action: { type: 'message', label: '完整分析', text: `分析${mtInfo && mtInfo.displayNum ? mtInfo.displayNum : tableName.replace(/^百家樂\s*/, '')}` },
      height: 'sm', flex: 1
    }]
  });
  ...
  return {
    type: 'flex',
    altText: `✅ 已跟隨 ${tableName}`,
    quickReply: QUICK_REPLY,
    contents: {
      type: 'bubble',
      header: {
        type: 'box', layout: 'vertical',
        backgroundColor: COLOR_HEADER, paddingAll: 'md',
        contents: [
          { type: 'text', text: `✅ 已跟隨`, color: '#ffffff', size: 'xs' },
          { type: 'text', text: tableName, color: '#ffffff', size: 'lg', weight: 'bold' },
          { type: 'text', text: `荷官：${dealer}`, color: '#aaaacc', size: 'xs', margin: 'xs' }
        ]
      },
      body: {
...
module.exports = { buildAnalysisFlex, buildHandResultFlex, buildRoomCarousel, buildTrackingFlex, QUICK_REPLY };
